...
// RenderArticlesPageV2 渲染文章列表页面
func RenderArticlesPageV2(c *gin.Context) {
	param := GetArticlesPageRequest{
		Page:     cast.ToInt(c.DefaultQuery("page", "1")),
		PageSize: cast.ToInt(c.DefaultQuery("pageSize", "20")),
		Search:   c.Query("search"),
	...
	c.HTML(http.StatusOK, "list", templateData)
}

// RenderArticleDetailV2 渲染文章详情页面
func RenderArticleDetailV2(c *gin.Context) {
	id := cast.ToUint64(c.Param("id"))
	if id == 0 {
		c.HTML(http.StatusNotFound, "error.html", gin.H{
			"title":   "页面不存在",
	...
	c.HTML(http.StatusOK, "detail", templateData)
}
