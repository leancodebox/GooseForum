...
采用“访问组授予分类能力，主分类决定主题可见性”的模型：

```text
用户 -> 访问组成员关系 -> 分类能力 -> 主题主分类 -> 帖子
```
...
5. 公开分类可选择一至三个；只要包含受限分类，该受限分类必须是主题的唯一分类。第一个分类仍是决定可见性的主分类。
...
| 模型 | 优点 | 尚未解决的问题 |
| --- | --- | --- |
| 分类权限 | 符合传统论坛板块和现有分类版主模型 | 多分类主题需要唯一、稳定的可见性来源 |
| 每主题可见性 | 跨列表可通过一个索引列过滤，分类保持纯归类 | 引入第二个近似“分类”的概念，也不能单独隐藏/保护分类 |
| 主分类 + 附加分类 | 可见性是单值判定；列表、详情、搜索使用同一索引列 | 分类顺序具有业务含义，变更主分类需要明确提示和权限校验 |

最终选择 `topics.main_category_id`：它是 `category_ids` 第一个非零值的索引化副本。分类仍承载访问能力，但一个主题的所有读取入口只检查主分类。这样分类权限变化不会反向制造非法主题组合，也不需要公开转受限的批量重写、冲突预览或转换策略。
...
组 ID 不复制进 topics、posts、通知、文件或搜索文档；这些资源沿 `resource -> post/topic -> main_category_id` 解析权限。
...
AND topics.main_category_id = category_ids 中第一个非零值
AND 选中的分类若有任意一个受限，category_ids 只能包含该分类
```

新建、草稿和编辑都在服务端校验。前端选择限制只是交互帮助，不是授权。

**可见性只来自主分类。** `category_ids` 的第一项是主分类，写入 `topics.main_category_id`；当所有选中分类都是公开分类时，其余分类是纯粹的辅助标签。因此：

- 读判定在所有位置都是同一个条件——`topics.main_category_id IN (可读分类)`，作用在单个带索引的列上。列表与详情不可能给出不同答案。
- 受限分类必须单选。这是数据不变量，不是只存在于发布界面的交互规则；普通发布、管理端编辑和最终持久化都必须独立校验。
- 公开分类转为受限分类前，必须先消除它所在的多分类主题；管理写入会在同一事务内检查并在冲突时拒绝收窄权限。
...
分类路由先调用 `CanReadCategory`。由于被浏览的分类可能只是主题的辅助标签，列表 SQL 仍按 `main_category_id` 过滤当前受众；游客和普通注册用户可按稳定 audience 共享缓存，自定义组受众不建立列表缓存。

主题详情先按主键加载主题，在内存中检查一个主分类 ID，通过后才读取帖子窗口。帖子窗口 SQL 不加 ACL：
...
```sql
AND topics.main_category_id IN (:readable_category_ids)
```

不能先 `LIMIT` 再在 Go 中删除无权项，否则会出现稀疏页、错误游标和摘要泄漏。仓储层接收排序去重后的 ID；空集合直接返回空结果，覆盖全部有效分类时省略受众过滤。

会携带主题/回复预览的 `user_activities` 需要冗余 `topic_id`，使数据库能在游标分页前过滤；历史数据根据 subject 回填。

### 首页发现

当前采用“成员首页混排其可读受限主题”（原讨论方案 A），因为此时不需要先开发另一套私密内容更新发现机制：

- 游客和普通注册用户可以继续共享稳定的基础列表缓存。
- 额外拥有受限分类读取权的用户执行带 `main_category_id IN (...)` 的查询，不建立 audience 列表缓存。
- 分类页鉴权后仍按分类共享缓存。
...
- 搜索文档保存标量 `mainCategory`，查询使用当前请求的可读分类 ID；增加该字段的迁移必须重建已有搜索文档。
- 爬虫等同游客。
...
发布 Payload 下发用户可读取的分类，并标注 `canCreate/isRestricted`。界面明确标识第一个选择为主分类；选择受限分类时直接替换已选列表，从受限分类切回公开分类时也从新的单选开始。服务端仍独立执行完整授权和分类不变量校验。
...
- 应用迁移 v17 清理旧数据：受限主分类保留主分类并移除全部附加分类；公开主分类保留所有公开分类并移除受限附加分类。迁移同步修复 `topic_category_index` 和搜索文档，失败则不提升迁移版本。
...
- 增加访问组、成员和分类授权管理。
- 增加邀请/申请流程。
- 公开分类转受限前检查多分类主题冲突，冲突消除后才保存 grant。
- 图片通过 `file_usage -> post/topic -> category` 鉴权；新上传图片在被正文认领前仅上传者可读。

性能基线与复现命令见 [访问控制性能基线](topic-access-control-performance.md)。
分类页根据关联索引的基数自适应选择查询：稀疏分类先取有上限的 topic ID 再过滤、排序和分页；稠密分类使用 JOIN 让数据库完成同样的工作。稀疏路径的候选集被硬性限制，不对无界数据做 Go 层分页。
