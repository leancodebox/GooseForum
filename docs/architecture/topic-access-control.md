...
AND 只要选中任一受限分类，分类集合就只能包含该分类
```

新建、草稿和编辑都在服务端校验。前端选择限制只是交互帮助，不是授权。

公开/受限分类之间移动主题会改变整个主题及回复的可见性，只允许 `manage` 操作者执行，需要二次确认和审计。
...
- 删除 `everyone -> read` 不是普通开关。分类转受限前，管理员必须处理同时拥有其他分类的历史主题，统一选择“只保留本分类”或“移除本分类”。
- 转换过程锁定冲突主题并限制单次最多 10,000 个，分类 JSON 与索引行保持事务一致；全部整理完成后才收窄权限。
