...
5. 主题可选择一至三个分类；第一个是决定可见性的主分类，其余是辅助标签。
...
发布 Payload 下发用户可读取的分类，并标注 `canCreate/isRestricted`。界面明确标识第一个选择为主分类；移除主分类会先展示将被提升的新主分类并要求确认。服务端仍独立执行完整授权。
