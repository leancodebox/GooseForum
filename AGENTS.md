...
- 通过清晰的业务逻辑保证可靠性，不为断电保护引入恢复机制或持久化任务；禁止非必要事务，只有确实需要保证业务数据一致性时才使用事务。

- 代码以可维护为优先：改动保持简洁，命名和结构要清晰；在代码可读性、复用性之间保持平衡，不为了复用而过度抽象，也不重复堆业务逻辑。
- 使用 GORM 做局部字段查询时，优先定义用途明确的小型读模型，通过 `Model(&完整模型{}).Find(&小模型)` 让 GORM 根据目标类型推导查询列；避免在业务代码中使用 `Select(...)` 维护字段列表。仅使用 `Table(...).Find(&小模型)` 仍可能生成 `SELECT *`，不能作为部分字段查询。
...
- `db.default.connection`：sqlite、mysql 或 postgres
