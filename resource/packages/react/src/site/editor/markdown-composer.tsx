    ...
    <div>
      ...
      >
        ...
        {preview ? (
          value.trim() ? (
            <div
              className={cn("gf-prose gf-prose-post py-4", minHeight)}
              dangerouslySetInnerHTML={{ __html: renderMarkdown(value) }}
            />
          ) : (
            <p className={cn("py-4 text-sm text-muted-foreground", minHeight)}>
              {t("emptyPreview")}
            </p>
          )
        ) : mode === "visual" ? (
          ...
          <Textarea
            ...
            className={cn(
              "resize-y rounded-none border-0 px-1 py-4 font-mono shadow-none focus-visible:ring-0",
              minHeight,
            )}
