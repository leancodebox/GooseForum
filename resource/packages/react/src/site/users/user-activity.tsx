...
export function UserActivity({ page }: { page: UserProfileProps }) {
  ...
  }, [loading, page.activityTab, pagination, runtime, t]);
          ...
          <div className="flex flex-col gap-3 p-4">
            {likes.map((like) => (
              <GooseLink
                key={like.id}
                href={like.url}
                className="flex min-w-0 gap-3 rounded-lg border p-3 transition hover:border-primary/20 hover:bg-muted/40"
              >
                ...
                <span className="min-w-0">
                  ...
                  <time className="mt-1 block text-xs text-muted-foreground">
                    {formatDateTime(like.likedAt, runtime.locale)}
                  </time>
                </span>
              </GooseLink>
            ))}
...
export function UserActivityItem({
  ...
  return (
    <GooseLink
      href={activity.url || "#"}
      className="flex gap-3 rounded-lg border p-3 transition hover:border-primary/20 hover:bg-muted/40"
    >
      <span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground">
