...
export function SponsorsPageView({ page }: { page: SponsorsPageProps }) {
  ...
  return (
    <div className="pb-12">
      <PageHeader title={page.content.title} description={page.content.description} badge={<Badge variant="secondary">{page.totalCount}</Badge>} />
            ...
            <section key={section.key} className="flex flex-col gap-2.5">
              ...
              <div className={sectionGrid(section)}>
                {section.sponsors.map((sponsor) => {
                  ...
                  const className = cn('group flex items-start gap-2.5 rounded-xl border bg-background transition-colors hover:border-primary/30 hover:bg-primary/5', cardPadding(section))
                  return sponsor.link
...
function sectionGrid(section: SponsorSectionPayload) {
  return cn(
    'grid gap-3',
    section.tone === 'diamond' && 'grid-cols-1 lg:grid-cols-2',
    section.tone === 'gold' && 'grid-cols-1 lg:grid-cols-2 xl:grid-cols-3',
    section.tone !== 'diamond' && section.tone !== 'gold' && 'grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5',
  )
}
