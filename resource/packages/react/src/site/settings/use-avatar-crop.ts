...
export function useAvatarCrop({
  ...
  useEffect(() => {
    if (!open || !sourceUrl || !imageRef.current) return;
    let active = true;
  ...
  async function upload() {
    ...
    setCropError("");
    try {
      const large = await selection.$toCanvas({ width: 300, height: 300 });
...
async function centerSelection(cropper: import("cropperjs").default) {
