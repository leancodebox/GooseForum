...
const primaryItems = computed(() => props.layout.sidebar.main)
const resourceItems = computed(() => props.layout.sidebar.resources)
const headerResourceItems = computed(() =>
  ['sponsors', 'links']
    .map((key) => resourceItems.value.find((item) => item.key === key))
    .filter((item): item is NonNullable<typeof item> => Boolean(item)),
)
const footerLinks = computed(() => props.layout.footer.links)
const footerPrimary = computed(() => props.layout.footer.primary)
...
<template>
  <div class="min-h-screen bg-gray-50 text-gray-900">
    ...
    >
      ...
      <aside v-if="rail" class="hidden min-w-0 xl:block">
        <slot name="rail" />
      </aside>
    </main>

    <div v-if="drawerOpen" class="fixed inset-0 z-[60] lg:hidden" role="dialog" aria-modal="true">
      <button class="absolute inset-0 bg-gray-900/40" :aria-label="t('shell.closeMenu')" @click="closeDrawer" />
      <nav class="relative h-full w-80 max-w-[85vw] overflow-y-auto bg-white p-3 shadow-xl">
        <div class="mb-3 flex h-10 items-center justify-between">
          <div class="font-bold text-gray-900">{{ t('shell.menu') }}</div>
          <button class="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-500 hover:bg-gray-100" type="button" :aria-label="t('shell.closeMenu')" @click="closeDrawer">
            <X class="h-5 w-5" />
          </button>
        </div>
        <div class="space-y-0.5">
        ...
        <div v-if="layout.sidebar.categories.length" class="mt-5 space-y-0.5">
          <div class="px-2 text-[10px] font-bold uppercase tracking-wide text-gray-500">{{ t('shell.categories') }}</div>
          <a
            v-for="category in layout.sidebar.categories"
            :key="category.key"
            :href="category.url"
