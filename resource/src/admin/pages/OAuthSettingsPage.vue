...
<template>
  <BasicPage :title="adminText('k00fl')" :description="adminText('k00fm')">
    ...
    <form v-else-if="activeProvider" class="min-w-0" @submit.prevent="save">
      <Tabs v-model="activeProviderIndex" class="gap-6">
        <div class="min-w-0 overflow-x-auto pb-1">
          <TabsList :aria-label="adminText('k00fl')">
            <TabsTrigger
        ...
        <TabsContent :key="activeProviderIndex" :value="activeProviderIndex" class="flex flex-col gap-6">
          ...
          <div v-if="activeProvider.kind === 'oidc'" class="grid gap-6 md:grid-cols-2">
