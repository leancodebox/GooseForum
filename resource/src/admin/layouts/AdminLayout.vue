<template>
  <div class="drawer lg:drawer-open">
    ...
    <div class="drawer-content flex flex-col">
          ...
          <div class="dropdown dropdown-end">
            <div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar">
              <div class="w-8 rounded-full">
                <img :src="userStore.user?.avatar || '/static/pic/default-avatar.png'" :alt="userStore.user?.username" />
              </div>
            </div>
      ...
      <!-- 面包屑导航 -->
      <div class="bg-base-100 px-6 py-3 border-b border-base-300" v-if="breadcrumbs.length > 0">
        <div class="breadcrumbs text-sm">
          <ul>
            <li v-for="(crumb, index) in breadcrumbs" :key="index">
              <router-link v-if="crumb.path && index < breadcrumbs.length - 1" :to="crumb.path" class="link link-hover">
                {{ crumb.name }}
              </router-link>
    ...
    <div class="drawer-side">
      ...
      <aside :class="[
        ...
        <!-- 菜单 -->
        <ul class="menu px-2 py-4 space-y-2 w-full">
          <li v-for="item in menuItems" :key="item.key">
            <router-link 
              :to="item.path" 
              :class="[
                'flex items-center gap-3 px-3 py-3 mx-2 rounded-lg transition-colors',
                {
