  ...
  <nav :aria-label="t('topicList.mode.pagination')" class="border-t border-line bg-base-200/50 p-3 text-center">
    <template v-if="mode === 'pagination'">
      <div class="flex flex-wrap items-center justify-center gap-2">
        <Button v-if="previousUrl" as-child variant="surface" size="sm">
        ...
        <Button v-if="pagination.hasNext" as-child variant="surface" size="sm">
          <a :href="pagination.nextUrl" rel="next">
            {{ t('common.nextPage') }}
    ...
    <template v-else>
      <Button v-if="pagination.hasNext" as-child variant="brand-ghost" size="sm" class="gap-2 aria-disabled:cursor-wait">
        <a
