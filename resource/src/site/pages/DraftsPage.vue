...
import { FileText, PenSquare, Send, ShieldAlert } from '@lucide/vue'
...
<template>
  <main class="min-w-0 pb-8">
    ...
    <section class="overflow-hidden rounded-lg border border-gray-200/70 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
      <div class="hidden grid-cols-[minmax(0,1fr)_128px_188px] gap-3 border-b border-gray-100 bg-gray-50/60 px-4 py-2 text-[11px] font-bold uppercase text-gray-600 md:grid">
      ...
      <div v-if="props.drafts.length" class="divide-y divide-gray-100">
        <article
          ...
          :key="draft.id"
          class="grid gap-3 px-4 py-3 transition hover:bg-gray-50/70 md:grid-cols-[minmax(0,1fr)_128px_188px] md:items-start"
        >
          <div class="min-w-0">
            <div class="flex min-w-0 flex-wrap items-center gap-2">
              <a :href="draft.editUrl" class="truncate text-base font-semibold text-gray-950 hover:text-blue-600">
            ...
            <div v-if="draft.categories.length" class="mt-2 flex flex-wrap gap-1.5">
              <span
                v-for="category in draft.categories"
                :key="category.id"
                class="inline-flex items-center gap-1 rounded-full border border-gray-200 px-2 py-0.5 text-[11px] font-medium text-gray-600"
              >
          ...
          <time class="text-xs font-medium text-gray-400 md:pt-1 md:text-right">{{ formatDateTime(draft.updatedAt) }}</time>
          <div class="flex flex-wrap items-center gap-2 md:flex-nowrap md:justify-end">
            <a
              :href="draft.editUrl"
              class="inline-flex h-8 shrink-0 items-center gap-1 rounded-md border border-gray-200 px-2.5 text-xs font-semibold text-gray-600 hover:bg-white hover:text-gray-900"
            >
