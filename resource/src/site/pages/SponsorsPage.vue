      ...
      <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_260px]">
        <div class="space-y-5">
          <section v-for="section in props.sections" :key="section.key" class="space-y-3">
            <div class="flex items-center justify-between gap-3 border-b border-line pb-2">
              ...
              <span class="rounded-full bg-base-300 px-2 py-0.5 text-[11px] font-semibold text-base-content/55">{{ section.sponsors.length }}</span>
            </div>
            ...
            <div class="grid gap-3" :class="sectionGrid(section)">
              <a
                ...
                class="group relative rounded-lg border border-line bg-base-100 transition hover:border-primary/20 hover:bg-info/10"
          ...
          <div v-if="!hasSponsors" class="rounded-lg border border-line bg-base-100 px-5 py-16 text-center">
        ...
        <aside class="space-y-3">
          <div class="rounded-lg border border-line/70 bg-base-100 p-4">
            ...
            <a :href="props.contact.buttonLink" class="mt-4 inline-flex h-9 items-center gap-1.5 rounded-md bg-primary px-3 text-sm font-semibold text-primary-content hover:bg-primary">
              <Mail class="h-4 w-4" />
              {{ props.contact.buttonText }}
            </a>
          </div>

          <div v-if="props.rules.length" class="rounded-lg border border-line/70 bg-base-100 p-4">
