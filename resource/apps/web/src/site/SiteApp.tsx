...
import { GoosePage, isStandalonePage } from "@gooseforum/theme-default/app/root";
import { AppShell } from "@gooseforum/theme-default/site/layout/app-shell";
import { BootstrapLoading } from "@gooseforum/ui/bootstrap";
...
export function SiteApp({ pageSource, initialPage }: SiteAppProps) {
  ...
  const [cachedPages, setCachedPages] = useState<CachedPage[]>(() =>
    initialPage && keepAliveComponents.has(initialPage.component)
      ? [{ key: routeCacheKey(new URL(initialPage.url, window.location.href)), page: initialPage }]
      : [],
  );
  ...
  const setLocale = useCallback(async (nextLocale: AuthLocale) => {
  ...
  const pageContent = page ? (
    <>
      ...
      {error ? <BrowserBootstrapError error={error} onRetry={() => void refresh()} retrying={isNavigating} /> : !activeCacheKey ? <GoosePage page={page} /> : null}
    </>
  ) : null;
