...
import { BootstrapError, BootstrapLoading, GooseApp } from '@gooseforum/react/app'
import {
...
export function SiteApp({ pageSource }: SiteAppProps) {
  ...
  const loadPage = useCallback(async (
    ...
    try {
      ...
      } else if (historyAction === 'replace' || nextUrl.href !== window.location.href) {
  ...
  const navigate = useCallback(async (href: string, options: GooseNavigateOptions = {}) => {
  ...
  const runtime = useMemo<GooseRuntime>(() => ({
    ...
    setLocale,
    toggleTheme,
  }), [isNavigating, locale, navigate, page?.url, pageSource.api, refresh, setLocale, theme, toggleTheme])

  return (
    <GooseI18nProvider locale={locale}>
      <GooseRuntimeProvider runtime={runtime}>
        {error
          ? <BootstrapError error={error} onRetry={() => void refresh()} retrying={isNavigating} />
          : page
            ? <GooseApp page={page} />
            : <BootstrapLoading />}
      </GooseRuntimeProvider>
...
function browserUrlForPage(pageUrl: string, requestedUrl: URL) {
  ...
  const browserUrl = new URL(`${payloadUrl.pathname}${payloadUrl.search}`, window.location.origin)
